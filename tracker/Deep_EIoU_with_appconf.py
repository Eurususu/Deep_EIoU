...
#jjh
from tracker.confidence import EfficientAdaptiveAppearanceUpdater
...
class STrack(BaseTrack):
    ...
    def __init__(self, tlwh, score, feat_conf, feat=None, feat_history=60):
    ...
    def update_features(self, feat):
        ...
        if self.smooth_feat is None:
            ...
            # jjh
            self.smooth_feat = (
                                           1 - self.feat_conf * self.base_upgrade) * self.smooth_feat + self.feat_conf * self.base_upgrade * feat
...
class Deep_EIoU(object):
    def __init__(self, args, frame_rate=30):
        ...
        # jjh
        self.feature_updater = EfficientAdaptiveAppearanceUpdater()
        self.beta = 1.0
    ...
    def update(self, output_results, embedding):
        ...
        if len(output_results):
            ...
            # jjh 根据bboxs和scores计算出检测框的外观置信度
            app_confidence = self.feature_updater.compute_appearance_confidence(bboxes, scores)
            ...
            # jjh
            app_confidence_keep = app_confidence[remain_inds]
        ...
        if len(dets) > 0:
            ...
            if self.args.with_reid:
                ...
                detections = [STrack(STrack.tlbr_to_tlwh(tlbr), s, c, f) for
                              (tlbr, s, c, f) in zip(dets, scores_keep, app_confidence_keep, features_keep)]
            else:
        ...
        for iteration in range(num_iteration):
            ...
            if self.args.with_reid:
                ...
                # jjh
                # 外观距离自适应
                det_app_conf = np.array([det.feat_conf for det in detections])
                adjustment_factors = 1 + self.beta * (1 - det_app_conf)
                emb_dists = emb_dists * adjustment_factors[np.newaxis, :]
        ...
        if len(scores):
            ...
            # jjh
            app_confidence_second = app_confidence[inds_second]
            if self.args.with_reid:
        ...
        if len(dets_second) > 0:
            ...
            if self.args.with_reid:
                ...
                detections_second = [STrack(STrack.tlbr_to_tlwh(tlbr), s, c, f) for
                                     (tlbr, s, c, f) in zip(dets_second, scores_second, app_confidence_second, features_second)]
            else:
        ...
        if self.args.with_reid:
            ...
            # jjh
            unconfirmed_app_conf = np.array([det.feat_conf for det in detections])
            adjustment_factors = 1 + self.beta * (1 - unconfirmed_app_conf)
            emb_dists = emb_dists * adjustment_factors[np.newaxis, :]
