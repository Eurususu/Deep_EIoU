...
def get_distance(track1_id, track2_id, track1, track2, feat_threshold=0.7):
    """
    ...
    def filter_features(track):
        """筛选超过阈值的特征并计算平均特征向量"""
        valid_features = [feat for feat, score in zip(track.features, track.feat_scores) if score >= feat_threshold]
    ...
    if doesOverlap:
        ...
        feat1 = filter_features(track1)
        feat2 = filter_features(track2)
        if feat1 is None or feat2 is None:
            feat1 = np.mean(track1.features, axis=0)
            feat2 = np.mean(track2.features, axis=0)
        device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        t1 = torch.tensor(feat1, dtype=torch.float32).to(device)
        t2 = torch.tensor(feat2, dtype=torch.float32).to(device)
...
def split_trajectory(trklet, low_confidence_thresh=0.7, similarity_thresh=0.7, min_segment_length=5):
    ...
    # 4. 转换为帧号范围
    # result_segments = []
    # for seg in merged_low_segments:
    #     start_frame = frames[seg[0]]
    #     end_frame = frames[seg[1]]
    #     result_segments.append((start_frame, end_frame))
            ...
            if not id_switch_detected:
                ...
                for cluster in clusters:
                    if cluster[0] == 0:
                        ...
                        tmp_frames = frames[cluster[0]: cluster[1]+1]
                        tmp_scores = scores[cluster[0]: cluster[1]+1]
                        tmp_bboxes = bboxes[cluster[0]: cluster[1]+1]
                        tmp_embs = embs[cluster[0]: cluster[1]+1]
                        tmp_feat_scores = feat_scores[cluster[0]: cluster[1]+1]
                        tracklets[new_id] = Tracklet(new_id, tmp_frames.tolist(), tmp_scores.tolist(), tmp_bboxes.tolist(), feats=tmp_embs.tolist(),  feat_scores=tmp_feat_scores.tolist())
                        new_id += 1
...
def parse_args():
    ...
    parser.add_argument('--max_k',
                        type=int,
                        default=10,
                        help='Maximum number of clusters/subtracklets to be output by splitting component.') # split的最大数量
